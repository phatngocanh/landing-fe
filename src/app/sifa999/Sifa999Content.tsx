...
export default function Sifa999Content() {
    ...
    <div className="scroll-smooth">
      ...
      <div className="bg-muted/50 border-b border-border">
        <div className="container py-3 md:py-4">
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs md:text-sm text-muted-foreground">
            ...
            <ChevronRight className="w-3.5 h-3.5 shrink-0" aria-hidden="true" />
            <span className="text-foreground font-semibold" aria-current="page">SIFA999</span>
          </nav>
        </div>
      </div>
      ...
      <section className="relative bg-green-900 text-white overflow-hidden">
        <div className="absolute inset-0">
          <Image src={heroBanner} alt="" fill className="object-cover opacity-20" aria-hidden="true" />
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
              {featuredProducts.map((p) => (
                <Link key={p.id} href={`/product/${p.id}`} data-testid={`product-card-${p.id}`} className="bg-card border border-border p-3 md:p-5 rounded-xl md:rounded-2xl text-center group hover:border-green-400 hover:shadow-md transition-all">
                  {p.badge && (
                    <div className="text-[11px] md:text-xs font-black bg-green-600 text-white px-2 py-0.5 rounded-full w-fit mx-auto mb-2">{p.badge}</div>
                  )}
