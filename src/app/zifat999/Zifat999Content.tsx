      ...
      <section className="relative bg-teal-900 text-white overflow-hidden">
        ...
        <div className="relative container py-16 md:py-24">
          <div className="mb-5 flex items-center gap-4">
            <BrandLogo brand="ZIFAT999" size="lg" className="border-teal-200/70 bg-white/95 ring-teal-200" />
            <span className="inline-block px-4 py-1.5 rounded-full bg-teal-600 text-white text-[11px] font-black uppercase tracking-widest">
              ZIFAT999
            </span>
          </div>
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 bg-teal-600 text-white px-8 py-4 rounded-full font-bold uppercase tracking-wider text-sm hover:brightness-110 transition-all shadow-xl"
          >
      ...
      <main id="main-content" className="container py-8 md:py-16">
        ...
        <ScrollReveal>
          <div className="mb-10 md:mb-16">
            <h2 className="section-header-line text-sm font-black uppercase tracking-[0.25em] text-teal-600 mb-5 md:mb-6">
              Danh Mục Sản Phẩm
            </h2>
            <div className="flex flex-wrap gap-2 md:gap-3">
              {categories.map((cat) => (
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-4 py-2 text-sm font-medium text-foreground transition-all hover:border-teal-400 hover:text-teal-600 hover:shadow-sm active:scale-95 md:px-5 md:py-2.5"
                >
                  {cat.name}
        ...
        <ScrollReveal>
          <div className="mb-12 md:mb-20">
            <div className="flex items-center justify-between border-b border-border pb-4 mb-6 md:mb-8">
              <h2 className="text-sm font-black uppercase tracking-[0.25em] text-teal-600">
                Sản Phẩm Nổi Bật
              </h2>
              <Link href="/products?brand=ZIFAT999" className="shrink-0 text-[13px] text-muted-foreground font-bold hover:text-teal-600 transition-all flex items-center gap-1.5 group whitespace-nowrap">
                Tất cả <span className="group-hover:translate-x-1 transition-transform">→</span>
              </Link>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
              {featuredProducts.map((p) => (
                <Link key={p.id} href={`/product/${p.id}`} data-testid={`product-card-${p.id}`} className="bg-card border border-border p-3 md:p-5 rounded-xl md:rounded-2xl text-center group hover:border-teal-400 hover:shadow-md transition-all">
                  {p.badge && (
                    <div className="text-[11px] md:text-xs font-black bg-teal-600 text-white px-2 py-0.5 rounded-full w-fit mx-auto mb-2">{p.badge}</div>
                  )}
                  ...
                  <h3 className="text-[11px] md:text-sm font-bold text-foreground line-clamp-2 mb-1 group-hover:text-teal-600 transition-colors">{p.name}</h3>
                  <p className="text-teal-600 font-black text-sm md:text-base">{p.price}</p>
        ...
        <ScrollReveal>
          <div className="mb-16 md:mb-24">
            <h2 className="section-header-line text-sm font-black uppercase tracking-[0.25em] text-teal-600 mb-8">
              Khách Hàng Nói Gì
            </h2>
        ...
        <ScrollReveal>
          <div className="bg-teal-900 text-white rounded-2xl md:rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-teal-200 text-sm md:text-base max-w-lg">
                Tham gia mạng lưới phân phối ZIFAT999 với chính sách hấp dẫn, hỗ trợ marketing toàn diện.
              </p>
            </div>
            <div className="flex gap-3">
              <Link href="/contact" className="bg-teal-600 text-white px-6 md:px-8 py-3 rounded-full font-bold text-sm hover:brightness-110 transition-all shadow-lg">
                Liên hệ ngay
              </Link>
              <a href="tel:02862713214" className="flex items-center gap-2 border border-teal-400 text-teal-200 px-6 py-3 rounded-full font-bold text-sm hover:bg-teal-800 transition-all">
