...
export default function Zifat999ContactPage() {
  return (
    <div className="scroll-smooth">
      ...
      <main className="container py-10 md:py-16">
        <h1 className="text-2xl md:text-4xl font-black text-foreground mb-4">Liên Hệ ZIFAT999</h1>
        <p className="text-muted-foreground mb-10 max-w-2xl">Đội ngũ ZIFAT999 luôn sẵn sàng tư vấn, báo giá sỉ và hỗ trợ bạn tìm giải pháp tẩy rửa phù hợp nhất.</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
            ...
            {[
              ...
              <div key={label} className="flex items-start gap-4 bg-card border border-border rounded-2xl p-5">
                ...
                <div>
                  ...
                  {href ? (
                    <a href={href} className="text-sm font-bold text-foreground hover:text-blue-600 transition-colors">{value}</a>
                  ) : (
          ...
          <div className="bg-card border border-border rounded-2xl p-6 md:p-8">
            <h2 className="font-black text-foreground text-lg mb-6">Gửi Yêu Cầu Báo Giá</h2>
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              <div>
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1.5 block">Họ tên</label>
                <input className="w-full px-4 py-2.5 bg-muted border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/30" placeholder="Nhập họ tên..." />
              </div>
              <div>
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1.5 block">Số điện thoại</label>
                <input className="w-full px-4 py-2.5 bg-muted border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/30" placeholder="Nhập SĐT..." />
              </div>
              <div>
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1.5 block">Nội dung</label>
                <textarea rows={4} className="w-full px-4 py-2.5 bg-muted border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/30 resize-none" placeholder="Mô tả nhu cầu của bạn..." />
              </div>
              <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold text-sm uppercase tracking-wider hover:brightness-110 transition-all">
                Gửi yêu cầu
              </button>
            </form>
          </div>
