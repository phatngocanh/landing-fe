...
const ProductsSection = async () => {
  const { items: products } = await listProducts({ pageSize: 8 });
  ...
  return (
    <section className="scroll-mt-28" id="products">
      ...
      {displayProducts.length === 0 ? (
        <p className="text-sm text-muted-foreground py-12 text-center">Chưa có sản phẩm.</p>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 md:gap-4">
          {displayProducts.map((p, i) => (
            <ScrollReveal key={p.id} delay={`${i * 50}ms`}>
              <Link href={`/product/${p.slug}`} className="bg-card border border-border p-1.5 sm:p-2 md:p-3 rounded-lg sm:rounded-xl text-center product-card group cursor-pointer h-full relative block">
                {p.badge && (
