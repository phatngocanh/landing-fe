        ...
        <ScrollReveal key={c.id} delay={`${i * 100}ms`}>
          <Link href={`/product/${c.id}`} className="bg-card rounded-xl sm:rounded-3xl overflow-hidden product-card group h-full block">
            <div className="relative aspect-square p-2 sm:p-6 bg-muted flex items-center justify-center overflow-hidden">
              <Image className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300" src={c.img} alt={c.name} placeholder="blur" width={512} height={512} />
              <div className="absolute top-1.5 left-1.5 sm:top-4 sm:left-4 bg-secondary text-secondary-foreground text-[11px] font-black px-1.5 py-0.5 sm:px-3 sm:py-1.5 rounded-full shadow-xl">{c.discount}</div>
              {c.badge && (
                <div className="absolute top-1.5 right-1.5 sm:top-4 sm:right-4 bg-primary text-primary-foreground text-[11px] font-black px-1.5 py-0.5 sm:px-3 sm:py-1.5 rounded-full">{c.badge}</div>
              )}
            ...
            <div className="p-2.5 sm:p-5 lg:p-6 text-center">
              <h3 className="text-[11px] sm:text-[13px] lg:text-[15px] font-bold overflow-hidden mb-1.5 sm:mb-3 text-foreground group-hover:text-primary transition-colors line-clamp-2 leading-snug">{c.name}</h3>
              <div className="flex flex-col items-center gap-0">
                <span className="text-secondary font-black text-sm sm:text-xl lg:text-2xl">{c.price}</span>
                <span className="text-muted-foreground text-[11px] sm:text-xs line-through">{c.oldPrice}</span>
              </div>
              <button className="mt-2 sm:mt-5 w-full bg-foreground text-card py-2 sm:py-3.5 rounded-full text-[11px] font-black uppercase tracking-wider sm:tracking-widest hover:bg-primary hover:text-primary-foreground transition-all active:scale-95 shadow-md hover:shadow-lg" aria-label={`Mua ngay: ${c.name}`}>Mua ngay</button>
            </div>
          </Link>
        </ScrollReveal>
      ))}
