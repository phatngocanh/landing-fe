  ...
  <ScrollReveal>
    <section className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-12 bg-card p-5 md:p-14 rounded-2xl md:rounded-3xl border border-border shadow-sm scroll-mt-28" id="news">
      <div className="space-y-4 md:space-y-8">
        ...
        <div className="aspect-video bg-foreground rounded-xl md:rounded-2xl overflow-hidden relative group cursor-pointer shadow-xl border border-border">
          <Image className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-500" src={heroBanner} placeholder="blur" alt="Video giới thiệu Phát Ngọc Anh" fill sizes="(max-width: 768px) 100vw, 33vw" />
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-12 h-12 md:w-16 md:h-16 bg-card/20 backdrop-blur-md rounded-full flex items-center justify-center group-hover:scale-125 transition-transform duration-500 shadow-2xl border border-card/30">
              <Play className="w-6 h-6 md:w-8 md:h-8 text-primary-foreground fill-current" aria-label="Phát video" />
            </div>
          </div>
        </div>
      </div>
